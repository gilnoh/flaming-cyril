...
easier for future researchers and developers to change analysis
...
a minimal number of analyzers, coupled with a set of basic
...
real-world in the three languages and were produced by native
speakers. 
All T-H pairs %of the three languages
are sampled from application gold data which were manually constructed
on the basis of anonymized real-world customer interactions datasets
(Eichler et al. \shortcite{Eichler:2014} for German;
REF\footnote{Omitted for anonymity. The dataset is publicly
available.} for English and Italian). The sets are fairly large (5300
pairs for English, 1700 for Italian, 1274 for German), and were
sampled to be balanced (random baseline is F$_1$=50). We report F$_1$
for comparability with non-balanced setups. 
...
with TIE for German. On English, BIUTEE still outperforms {\it
  MultiAlign} (-2). Thus, MultiAlign also performs acceptably
on real-world data. 
...
The paper proposed an approach to recognizing Textual Entailment
that is centered on the data structure of {{\em Multi-Level
    Alignments}.
The approach
  %closely follows successful alignment-based
%TE architectures, but
encourages having multiple alignments  
co-existing in a common data structure. 
We argued that this structure can provide an important
...
A pilot implementation %of this approach
showed performance comparable to other state-of-the-art open-source TE
engines.  
...
We believe that this approach can lead to more modular and more easily
